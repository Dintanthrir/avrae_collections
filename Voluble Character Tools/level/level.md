...
`!level` uses sources configured in a `voluble_config` variable but you can also specify a gvar to load and search for a class/subclass using `-source` to add homebrew which is not configured in a c/u/svar.